...
# Define the base directory where Sonarr configuration file is located if its attached via a mount point ex /mnt/tank
base_directory=""
...
cat <<EOT > $base_directory/configs/qbit/wireguard/wg0.conf
...
#restart qbit to use the new wireguard conf file
...
if [ -f "$base_directory/configs/prowlarr/config.xml" ]; then
  # Extract the API key using grep and awk
  prowlarr_api_key=$(grep -oP '(?<=<ApiKey>).*?(?=</ApiKey>)' "$base_directory/configs/prowlarr/config.xml")
...
# Check if the configuration file exists
if [ -f "$base_directory/configs/sonarr/config.xml" ]; then

  # Extract the API key using grep and awk
  sonarr_api_key=$(grep -oP '(?<=<ApiKey>).*?(?=</ApiKey>)' "$base_directory/configs/sonarr/config.xml")
...
# Check if the configuration file exists
if [ -f "$base_directory/configs/radarr/config.xml" ]; then

  # Extract the API key using grep and awk
  radarr_api_key=$(grep -oP '(?<=<ApiKey>).*?(?=</ApiKey>)' "$base_directory/configs/radarr/config.xml")
...
echo "$new_config" > "$base_directory/configs/qbit/config/qBittorrent.conf"
sudo docker start qbittorrent

#recyclarr setup
chown apps:apps -R /configs/recyclarr

echo "Shell into recyclarr and enter the command 'recyclarr config create'"
read -p "Press Enter when completed..."


cat <<EOF > $base_directory/configs/recyclarr/recyclarr.yml
